...
// filename 컬럼도 조회하도록 수정
$sql = "SELECT id, title, writer, created_at, filename FROM board $where ORDER BY id $order LIMIT $start, $limit";
          ...
          <tr>
            <td>
              <div class="title-with-file">
                <a href="view.php?id=<?= $row['id'] ?>&token=<?= $_GET['token'] ?>"><?= htmlspecialchars($row['title']) ?></a>
                <?php if ($row['filename']): ?>
